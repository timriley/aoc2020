...
var addendsLength = 25

func main() {
	var nums []int
	err := fileinput.LoadThen("day09/input.txt", "\n", func(s string) {
...
func part1(nums []int) int {
	for i := addendsLength; i < len(nums); i++ {
		num := nums[i]
		addends := nums[i-addendsLength : i]

		_, err := addendsForSum(addends, num)
		if err != nil {
			return num
		}
	}
