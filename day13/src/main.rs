...
//
// Your ferry can make it safely to a nearby port, but it won't get much further. When you
// call to book another ship, you discover that no ships embark from that port to your
// vacation island. You'll need to get from the port to the nearest airport.
//
// Fortunately, a shuttle bus service is available to bring you from the sea port to the
// airport! Each bus has an ID number that also indicates how often the bus leaves for the
// airport.
//
...
// departs at 0, 11, 22, 33, and so on. If you are there when the bus departs, you can
// ride that bus to the airport!
//
// Your notes (your puzzle input) consist of two lines. The first line is your estimate of
// the earliest timestamp you could depart on a bus. The second line lists the bus IDs
...
//
// The only bus departures that matter are the listed bus IDs at their specific offsets
// from t. Those bus IDs can depart at other times, and other bus IDs can depart at those
// times. For example, in the list above, because bus ID 19 must depart seven minutes
// after the timestamp at which bus ID 7 departs, bus ID 7 will always also be departing
...
// This is fine; the only requirement on that minute is that bus ID 19 departs then, and
// it does.
//
...
//
// However, with so many bus IDs in your list, surely the actual earliest timestamp will
// be larger than 100000000000000!
//
...
fn parse_input(input: &str) -> (usize, Vec<usize>) {
    ...
    let earliest_departure : usize = lines.next().expect("no timestamp found")
        .parse().expect("not a valid integer");

    let services : Vec<usize> = lines.next().expect("no services found")
        .split(",")
        .filter(|&s| s != "x")
...
fn next_service(earliest_departure: usize, services: &Vec<usize>) -> (usize, usize) {
